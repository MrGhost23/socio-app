...
const RecentActivity: React.FC<Props> = ({ userFirstName, recentActivity }) => {
  return (
    <div key={recentActivity.id} className="flex flex-row items-start gap-2">
      ...
      <p>
        {userFirstName}{" "}
        {recentActivity.action === "like" ? "liked" : "commented on"}
        <Link
          to={`/profile/${recentActivity.postAuthorId}`}
          className="font-medium"
        >
          {" "}
          {recentActivity.postAuthorFirstName +
            " " +
            recentActivity.postAuthorLastName}
        </Link>{" "}
        's
        <Link to={`/post/${recentActivity.postId}`} className="font-medium">
          {" "}
          post
        </Link>
        .
